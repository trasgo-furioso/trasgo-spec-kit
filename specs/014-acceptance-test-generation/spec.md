...
### User Story 4 - Testing Surface Contract Integration (Priority: P2)

When `testing-surface-*.md` contract files exist in the feature's `contracts/` directory (generated by `/speckit-plan`), the agent uses them as the authoritative source for page object selectors instead of inferring selectors from the frontend code. Each contract declares the parts (roles, accessible names, test IDs), states, and actions that a component exposes. The generated POM references only declared parts, and a provider-verification test is generated to assert that the live component satisfies its contract.

**Why this priority**: Without an explicit contract, POM selectors are best-effort guesses that drift as the UI evolves. The contract is the shared agreement between the test generator and the component developer -- it enables independent development of both sides.

**Independent Test**: Create a `contracts/testing-surface-checkout-form.md` with a Parts table declaring 3 parts (email, placeOrder, confirmation), run the command, and verify that the generated POM uses exactly those locators and that a provider-verification test asserts each part is present.

**Acceptance Scenarios**:

1. **Given** a feature directory with `contracts/testing-surface-checkout-form.md` declaring parts with roles, accessible names, and test IDs, **When** the agent generates tests, **Then** the POM for `CheckoutForm` uses only the declared locator strategies (role+name or testId) and does not infer selectors from the frontend code
2. **Given** a testing surface contract with a Parts table, **When** tests are generated, **Then** a provider-verification test file is created that asserts each declared part renders with the correct role, accessible name, and test ID
3. **Given** no `contracts/testing-surface-*.md` files in the feature directory, **When** the agent generates tests, **Then** it falls back to analyzing the frontend code and inferring selectors (User Story 1 behavior)
4. **Given** a testing surface contract declaring a part as `cardinality: zeroOrOne`, **When** tests are generated, **Then** assertions for that part use conditional checks rather than strict visibility assertions

---

### User Story 5 - Test Update on Spec Change (Priority: P3)
...
**Testing Surface Contracts**

- **FR-028**: The bundle MUST include a `testing-surface-contract` template in `bundle/presets/trasgospec/templates/` that defines the standard format for testing surface contracts (Parts table with columns: Part, Locator Strategy, Role, Accessible Name, Test ID, Cardinality; States table; Supported Actions table; Provider Obligations; Consumer Obligations). This template is resolved by `/speckit-plan` when generating UI contracts
- **FR-029**: When `contracts/testing-surface-*.md` files exist in the feature directory, the system MUST parse the Parts table and use declared locator strategies (role+name, label, testId) as the authoritative source for POM selectors
- **FR-030**: System MUST generate a provider-verification test file for each testing surface contract that asserts every declared part renders with the correct role, accessible name, and/or test ID
- **FR-031**: When no testing surface contracts exist, the system MUST fall back to frontend code analysis and selector inference (FR-005 through FR-007)
- **FR-032**: System MUST respect declared cardinality (`exactlyOne`, `zeroOrOne`, `many`) when generating assertions -- `exactlyOne` uses strict `toBeVisible()`, `zeroOrOne` uses conditional checks, `many` uses `toHaveCount()` with minimum bounds
...
- **Testing Surface Contract**: A Markdown file (`contracts/testing-surface-*.md`) generated by `/speckit-plan` that declares the parts, roles, test IDs, states, and actions a UI component exposes for testing. The shared agreement between the test generator and the component developer.
- **Provider Verification Test**: A generated test that asserts a component satisfies its testing surface contract at render time.
...
- When testing surface contracts are present, they take precedence over frontend code analysis for selector generation. The contract is the single source of truth
- The implementer agent may use Playwright MCP (`@playwright/mcp` with `--caps=testing`) to verify that generated selectors match the live DOM during implementation. This command does not invoke MCP itself -- it generates static test files

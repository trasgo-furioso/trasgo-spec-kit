...
- **FR-028**: When `contracts/testing-surface-*.md` files exist in the feature directory, the system MUST parse the Parts table and use declared locator strategies (role+name, label, testId) as the authoritative source for POM selectors
- **FR-029**: System MUST generate a provider-verification test file for each testing surface contract that asserts every declared part renders with the correct role, accessible name, and/or test ID
- **FR-030**: When no testing surface contracts exist, the system MUST fall back to frontend code analysis and selector inference (FR-005 through FR-007)
- **FR-031**: System MUST respect declared cardinality (`exactlyOne`, `zeroOrOne`, `many`) when generating assertions -- `exactlyOne` uses strict `toBeVisible()`, `zeroOrOne` uses conditional checks, `many` uses `toHaveCount()` with minimum bounds
...
- Testing surface contracts (`contracts/testing-surface-*.md`) are generated by `/speckit-plan` during Phase 1 when the project involves UI components. This command consumes them but does not create them
